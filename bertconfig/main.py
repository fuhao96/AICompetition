...
def train(model, device, trainLoader, opt, epoch):
    model.train()
    loss_sum, count = 0, 0
    progress_bar = tqdm(enumerate(trainLoader), total=len(trainLoader))  # 添加tqdm进度条
    for batch_index, batch_con in progress_bar:
        ...
        loss.backward()
        opt.step()
        loss_sum += loss.item()  # 使用loss.item()获取损失值
        count += 1

        if batch_index % 1000 == 999:
            avg_loss = loss_sum / count
            progress_bar.set_description(f"Epoch: {epoch + 1}/{args.epochs}, Train Loss: {avg_loss:.4f}")  # 更新进度条显示
            loss_sum, count = 0.0, 0

    if count > 0:  # 处理剩余的未满足1000步的损失
        avg_loss = loss_sum / count
        progress_bar.set_description(f"Epoch: {epoch + 1}/{args.epochs}, Train Loss: {avg_loss:.4f}")  # 更新进度条显示
